...
ProjT proj_apply_mult(ProjT base, float mult) {
  ProjT modified = base;
  modified.speed *= mult;
  modified.fire_rate = (u8)(base.fire_rate / mult);
  modified.fire_rate = modified.fire_rate < 1 ? 1 : modified.fire_rate;
  modified.targeting_max = (u16)(base.targeting_max * mult);
  modified.targeting_max = modified.targeting_max > PLAY_W ? PLAY_W : modified.targeting_max;
  return modified;
}
...
void proj_update(Proj* projectiles) {
  for (i8 i = 0; i < PROJ_MAX; i++) {
    ...
    ProjT pdata = proj_list[proj->type];
...
void particle_update(Particle* particles, Player p1) {
  for (i16 i = 0; i < PARTICLES_MAX; i++) {
    ...
    PosF p1_center = pos_addF(p1.pos, (PosF){4,4});
    if (part->life > 0 && distanceF(part->pos, p1_center) < p1.grab_distance) {
      // Move towards player
      part->vel = velocity_towards(part->pos, p1_center, 0.5f, 1);
    }
  }
}
...
void state_init(State* st) {
  ...
  for (i8 i = 0; i < PROJ_MAX; i++) {
    ...
    st->projectiles[i].type = 0;
    st->projectiles[i].mult = 1.0f;
  }
}
...
void input() {
  ...
  if (gamepad & BUTTON_1) {
    if (state.p1.b1_released) {
      ...
      state.p1.proj_index = state.p1.proj_index == state.p1.proj_slot_a ? state.p1.proj_slot_b : state.p1.proj_slot_a;
    }
...
void player_update() {
  ...
  // Projectiles
  ProjT pdata = proj_list[state.p1.proj_index];
  float mult = state.p1.proj_index == state.p1.proj_slot_a ? state.p1.proj_mult_a : state.p1.proj_mult_b;
  pdata = proj_apply_mult(pdata, mult);
  ...
  if (fire_frame && in_range) {
    for (i8 i = 0; i < PROJ_MAX; i++) {
      if (state.projectiles[i].pos.x == NULL_COORD) {
        state.projectiles[i].type = state.p1.proj_index;
...
void render() {
  ...
  *DRAW_COLORS = 1;
  // Slots
  bool is_slot_a = state.p1.proj_index == state.p1.proj_slot_a;
...
void update() {
  ...
  collisions_update();
  particle_update(state.particles, state.p1);
  render();
  // 
